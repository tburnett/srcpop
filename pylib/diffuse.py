...
from .skymaps import HPmap, _process_args
...
class Diffuse:
    ...
    def check_hpm(): # for superclass to invoke 
        pass 
    ...
    def plot_diffuse_flux(self,   figsize=(8,8)):
        ...
        hkw.pop('edgecolor', '')
        sns.histplot(data, ax=ax1, x='diffuse', **hkw, bins=25, kde=True, element='step')
        update_legend(ax1, data, hkw['hue'])
        ax1.set(**self.fluxticks('x') )
        
        sns.ecdfplot(data, ax=ax2, x='diffuse', legend=False,  **hkw)
        # sns.ecdfplot(ax=ax2, x=self.diffuse_hpm.map, label='isotropic', ls='--', color='grey')
        # sns.ecdfplot(ax=ax2, x=self.diffuse_hpm.map[self.ridge_mask], label='ridge', ls=':', color='grey')
        self.plot_limits( ax2)
