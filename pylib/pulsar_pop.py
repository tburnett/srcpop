...
def get_source_data(filename='files/classification.pkl'):
    ...
    introduction(df, filename)
    return df

def introduction(df, filename):
    show(f"""# Pulsar population studies""")
    show_date()
    show(f"""
        ### Two options:
        **A)** The unid's predicted to be pulsars are mostly undetected pulsars <br>
        **B)** There is a new, unknown galactic population of gamma-ray sources
         
        (Perhaps applicable philosophical principles: Occam's razor and the "duck" principle. Both would favor A.)
        ## Source selection
        Read in "{filename}" with {len(df)} source entries. This was generated by the 
        [ML classification analysis](machine_learning.ipynb).
...
def select_data(self=None, outfile=None):
    ...
    show(f"""
    Select two sets of "pulsar" sources: 
    1. Those identified as "PSR" or "MSP" in 4FGL-DR4: {t.IDed} 
    2. The unid's that were predicted to be pulsars: {t.predicted},  of which {nsgu} are tagged as SGUs<br>
    """)
    # Note that this includes {sum(pd.isna(data.set_index('pulsar_id').loc['predicted'].sgu))} 
    # UW sources **not** in 4FGL.
    # """)
...
def corner(data, fignum=1):
    show(r""" ## Source property corner plot 
        This "corner plot" shows the correlations of source properties grouped with the 
        spectral parameters `fpeak`, `curvature`, and `epeak` at the top, and galactic coordinates 
        `abs_sin_b` and `abs_glon`($|\sin(b)|$, $|l|$) and  at the bottom.""")
    (sns.PairGrid(data,hue='pulsar_id',corner=True,
            vars='log_fpeak curvature log_epeak abs_sin_b abs_glon'.split())
     .map_diag(sns.histplot,  kde=True, element='step')
     .map_lower(sns.kdeplot,)
     .add_legend(loc='upper right',bbox_to_anchor=(0.5, 0.95))
    );
    show(plt.gcf(), fignum=fignum)
                  
def plike(rec):
    class1 = rec.class1
    if not pd.isna(class1) and class1.lower() in ('msp','psr'): 
        return dict(msp='MSP', psr='RPP')[rec.class1.lower()]
    if rec.association=='unid' and rec.prediction=='psr': return 'unid'
...
def main():
    ...
    data = select_data()
    fignum=0
    corner(data, fignum=fignum+1)
    show_positions(data, fignum=fignum+1)
    spectral_correlations(data, fignum=fignum+1)
    sinb_vs_fpeak(data, fignum=fignum+1)          
    df = special(data, fignum=fignum+1)
    df.index.name = '4FGL-DR4'
